...
import { X, Delete, Calculator as CalcIcon, Copy, Check, Sparkles, LogOut } from "lucide-react";
...
export const DigitalCalculatorModal: React.FC<DigitalCalculatorModalProps> = ({
  ...
  // Allow ESC key to skip / dismiss calculator instantly
  useEffect(() => {
    ...
    >
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-xs shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-3.5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
          ...
          <button
            onClick={onClose}
            className="px-2 py-1 rounded-lg bg-slate-200/60 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-rose-50 hover:text-rose-600 text-[11px] font-bold transition-colors flex items-center gap-1"
        ...
        {/* Apply value trigger if requested or general dismiss footer */}
        <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 flex items-center gap-2">
          {onApplyValue ? (
            <>
              <button
                onClick={onClose}
                className="px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 font-bold text-xs transition-colors"
              >
                Skip
              </button>
              <button
                onClick={handleApply}
                className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs shadow-md shadow-blue-600/20 transition-all flex items-center justify-center gap-1.5"
              >
            ...
            <button
              onClick={onClose}
              className="w-full py-2 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs transition-colors flex items-center justify-center gap-1"
            >
